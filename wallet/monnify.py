...
class Monnify:
    def __init__(self, apiKey, clientSecretKey, is_test):
        self.apiKey = apiKey
    ...
    def generateToken(self):
        ...
        # print(headers)
        url = "https://sandbox.monnify.com/api/v1/auth/login" if self.is_test else "https://api.monnify.com/api/v1/auth/login"
    ...
    def createReserveAccount(self, accountReference, accountName, currencyCode, contractCode, customerEmail, customerName):
        rHeaders = {'Content-Type': "application/json",
                    'Authorization': "Bearer {0}".format(self.generateToken())}

        reserveAccUrl = "https://sandbox.monnify.com/api/v1/bank-transfer/reserved-accounts" if self.is_test else "https://api.monnify.com/api/v1/bank-transfer/reserved-accounts"
        data = {
        ...
        createReserveAccount = requests.post(
            reserveAccUrl, data=json.dumps(data), headers=rHeaders)
